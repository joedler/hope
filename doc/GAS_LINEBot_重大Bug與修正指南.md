...
# 案例：只重用單據產生，沒有重用完整業務流程

完整案例請見：`doc/非授課報酬完整流程復盤.md`。

症狀：

- 新功能看似能填資料與產生 PDF，但缺少試算、確認寫入、結算回看或寄送通知。
- Email 與 LINE 沒有分通道資格與回寫。
- GAS 已部署，但 GitHub Pages 仍是舊畫面。
- 分頁或範本採延遲建立，導致驗收時看不到資源。

修正原則：

1. 先畫出既有功能的完整狀態機，再決定重用哪些程式。
2. 試算、寫入、產生 PDF、Email、LINE 必須視為不同副作用。
3. 「介面簡單」不得解釋成移除預覽、確認、防重複或追蹤。
4. 同時檢查 GAS push、Web App 部署版本與 GitHub Pages。
5. 正式驗收需證明預覽不寫入、各階段不可跳過、通道獨立、補救工具相容。

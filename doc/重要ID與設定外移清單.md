...
目前程式仍保留部分 template/folder/admin fallback，目的是避免測試環境屬性缺漏時立即中斷。正式切換前應逐項確認 Script Properties 已建立，並在下一階段將正式版重要 ID fallback 移除或改為明確錯誤。

## 二、應放在 Google Sheets 的資料
...
2. Google Docs 範本權限允許 GAS 專案使用，`forceAuth()` 可完成授權。
3. PDF 資料夾權限允許 GAS 建立檔案。
4. `學生基本資料表` 已填入需要 LINE push 的家長 LINE User ID 與 Email。
5. `講師名單` 已填入講師 LINE User ID 與 Email。
6. 行政人員 LINE User ID 已放入 `ADMIN_LINE_USER_IDS` 或等效正式權限來源。
7. 正式版若移除程式 fallback，需先在測試 GAS 執行一次完整流程驗收。
